...
def goal_candidates(rows, history, allowed: set[str] | None = None,
    ...
    for entry in props:
        ...
        if unit == GOAL_SUM and money is None:
            continue   # в деньгах по этому товару шага нет — предлагать нечего
        saving = (round(spend - money, 2) if unit == GOAL_SUM
...
def _goal_normal_phrase(progress: dict, past: bool = False) -> str:
    ...
    if past:
        return f"обычная частота была {progress['normal']} раз в месяц"
    return f"обычная частота {progress['normal']} раз в месяц"
...
def goal_step_phrase(item: dict) -> str:
    ...
    return f"не чаще {item['target']} {times} в месяц (обычно {item['baseline']})"
...
def goal_proposals_text(candidates: list[dict], unit: str = GOAL_COUNT) -> str:
