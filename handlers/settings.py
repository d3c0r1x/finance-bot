...
async def send_settings(message: Message) -> None:
    await message.answer(await build_settings_text(), reply_markup=get_settings_kb())
...
async def settings_callback(callback: CallbackQuery):
    await callback.message.answer(await build_settings_text(), reply_markup=get_settings_kb())
    await callback.answer()


# ─── Бюджет ──────────────────────────────────────────────────────────────

async def build_budget_text() -> str:
    limits = await budget.get_limits()
    total_limit = await budget.get_total_limit()
...
async def send_budget(target_message: Message, edit: bool = False) -> None:
    text = await build_budget_text()
    keyboard = get_budget_kb(await budget.get_limits(), await budget.get_total_limit(), CATEGORIES)
    if edit:
        try:
...
async def budget_menu(callback: CallbackQuery):
    await send_budget(callback.message, edit=True)
    await callback.answer()
...
async def budget_quick_value(callback: CallbackQuery, state: FSMContext):
    ...
    await _apply_limit(callback.message, state, value)
...
async def budget_custom_value(message: Message, state: FSMContext):
    ...
    try:
        ...
        return
    await _apply_limit(message, state, value)


async def _apply_limit(message: Message, state: FSMContext, value: float) -> None:
    """Сохраняет новый лимит и показывает обновлённый бюджет."""
    ...
    if target == "total":
        await budget.set_total_limit(value)
        label = "Лимит на месяц"
    else:
        await budget.set_limit(target, value)
...
async def budget_reset(callback: CallbackQuery):
    await budget.reset_limits()
    await callback.answer("Вернул стартовые лимиты")
    await send_budget(callback.message, edit=True)
...
async def budget_ai(callback: CallbackQuery, state: FSMContext):
    ...
    context, days = await budget.history_summary(months=2)
    ...
    current_limits = await budget.get_limits()
...
async def budget_apply(callback: CallbackQuery, state: FSMContext):
    ...
    if not proposal:
        ...
        return
    await budget.apply_limits(proposal["limits"], proposal.get("total"))
    await state.update_data(budget_proposal=None)
    await callback.answer("Применил")
    await send_budget(callback.message)
